export default async function handler(req, res) {
  ...
  try {
    ...
    if (WEBHOOK_URL) {
      ...
      const memberFields = [];
      if (m2.name?.trim()) memberFields.push({ name: '👤 Member 2', value: `**${m2.name}** | ${m2.discord || '—'} | ${m2.email || '—'}`, inline: false });
      if (m3.name?.trim()) memberFields.push({ name: '👤 Member 3', value: `**${m3.name}** | ${m3.discord || '—'} | ${m3.email || '—'}`, inline: false });
      ...
      await fetch(WEBHOOK_URL, {
        ...
        body: JSON.stringify({
          embeds: [{
            ...
            footer:    { text: `RoboNexus '26 Registration System` },
